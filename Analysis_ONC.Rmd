...
*For details on bioinformatics on genotyping the data set see `Demultiplex.html` and `Genotyping.html`*
...
Presence of an isolation by distance effect among all tributary run groups was determined by testing for correlation between genetic distance (Fst/(1-Fst)) and river distance using a partial Mantel test to account for run type. Similarly, Mantel tests were used to test for isolation by distance among Fall and Spring runs only. Significance was determined using 9999 permutations.

Mantel tests test for correlation between two distance or dissimilarity matrices. Similarly, a partial Mantel test determines partial correlation between two matrices conditioned on a third matrix. This is accomplished by calculating the correlation of residuals for each of the two distance or dissimilarity matrices after a linear regression on the third matrix.

Significance of the calculated correlation coefficient is determine using permutations. For a mantel test only the first matrix is permuted, the second is kept constant. Similarly, for a partial Mantel test only the first matrix is permuted to ensure the correlation between the first and second matrices is kept constant.

```{r}

# read in fst matrix
gen_dist <- read_delim("data/POPGEN/all_dist_gen.csv", delim = ",") %>%
  column_to_rownames("rownames") %>%
  as.matrix()

# read in distance matrix
river_dist <- read_delim("data/POPGEN/all_dist_geog.csv", delim = ",") %>%
  column_to_rownames("rownames") %>%
  as.matrix()

# create strata to correct for run type
IBD_strata <- read_delim("data/POPGEN/pairwise_run_strata.txt", delim = "\t") %>%
  column_to_rownames("rownames") %>%
  as.matrix()

# test for for all pairs IBD using mantel test
mantel.partial(xdis = gen_dist,
               ydis = river_dist,
               zdis = IBD_strata, permutations = 9999)

```

Test for correlations between river distance and Fst within Fall run tributaries only using Mantel Test.

```{r}

# read in fst matrix
gen_dist <- read_delim("data/POPGEN/Fall_dist_gen.csv", delim = ",") %>%
  column_to_rownames("rownames") %>%
  as.matrix()

# read in distance matrix
river_dist <- read_delim("data/POPGEN/Fall_dist_geog.csv", delim = ",") %>%
  column_to_rownames("rownames") %>%
  as.matrix()

# test for IBD correcting for run type
mantel(xdis = gen_dist,
              ydis = river_dist,
              method = "pearson", permutations = 9999)

```

Look for IBD withing Spring populations.

```{r}

# read in fst matrix
gen_dist <- read_delim("data/POPGEN/Spring_dist_gen.csv", delim = ",") %>%
  column_to_rownames("rownames") %>%
  as.matrix()

# read in distance matrix
river_dist <- read_delim("data/POPGEN/Spring_dist_geog.csv", delim = ",") %>%
  column_to_rownames("rownames") %>%
  as.matrix()
...
Ne <- ldply(Ne, data.frame) %>%
  mutate(RUN_LOC = str_sub(`.id`, 9, -4)) %>%
  separate(RUN_LOC, into = c("RUN", "LOCATION"), remove = FALSE) %>%
  mutate(RUN = case_when(RUN == "F" ~ "Fall",
                         RUN == "S" ~ "Spring",
                         RUN == "L" ~ "Late-Fall",
                         RUN == "W" ~ "Winter"),
         SOURCE = ifelse(LOCATION %in% c("COL", "FRH", "NIM", "MRH"), "HATCHERY", "WILD"),
         ...
         LOCATION = ordered(LOCATION, levels = tributaries),
         RUN_LOC = ordered(RUN_LOC, levels = trib_runs),
         GENERATION_TIME = 1/(2*MIDPOINT_C))
  

kable(
  Ne %>%
    filter(MIDPOINT_C == 0.475) %>%
    select(RUN_LOC, NE, PARA_95_LOW, PARA_95_HIGH) %>%
    arrange(RUN_LOC),
  digits = 0,
  caption = "Table S3: Estimates of effective population size for tributary/run groups using LinkNe."
)

```

Negative estimates indicate that the sample size is too small to get an estimate. We'll code those as `NA` to be able to look at estimates of Ne over time.

```{r}

Ne <- Ne %>%
  mutate(NE = ifelse(NE < 0, NA, NE),
         PARA_95_HIGH = ifelse(PARA_95_HIGH < 0, NA, PARA_95_HIGH),
         PARA_95_LOW = ifelse(PARA_95_LOW < 0, NA, PARA_95_LOW))

```

Let's compare the change in effective population size over time.

```{r fig.cap="Change in effective population size Ne over time. Assuming linear change recombination rates and generation times are related as t = 1/(2c); for Chinook salmon generation times are approximately 6-8 years. For Fall runs in Deer and Mill Creek and Spring Feather River Hatchery sample sizes are too low relative to Ne to obtain estimates. For Spring Feather River Hatchery point estimates could only be obtaimed for most distant past (20 generation times) and has been excluded from this figure.", fig.height=21, echo=FALSE}

Ne %>%
  filter(!RUN_LOC %in% c("S_FRH")) %>%
  filter(!is.na(NE)) %>%
ggplot(aes(x = GENERATION_TIME, y = NE, color = RUN, shape = SOURCE)) +
  geom_line(size = .75) +
  geom_point(size = 1, stroke = 1, fill = "white") +
  facet_grid(RUN_LOC ~ ., scales = "free") +
  scale_color_manual(values = run_col) +
  scale_shape_manual(values = c(21, 22)) +
  labs(x = "generation times", y = "effective population size Ne") +
  theme_facet
...
kable(
    mean,
    digits = 4,
    caption = "Table S4: Mean, standard deviation, median, 25th, and 75th quartile of observed heterozygosity across loci for individuals grouped by run type & tributary."
    )
...
kable(
    mean,
    caption = "Table S5: Mean, standard deviation, median, 25th, and 75th quartile of observed heterozygosity per locus for individuals grouped by run type & tributary.",
    digits = 4
    )
...
kable(
  results %>%
    ...
    filter(stat > 0 & p_adj < 0.05) %>%
    arrange(desc(stat)),
  caption = "Table S6 Significance of pairwise comparisons of allele counts per locus for individuals grouped by run type within tributaries."
)


kable(
    mean,
    caption = "Table S7: Mean, standard deviation, median, 25th, and 75th quartile of allelic richness across loci for individuals grouped by run type & tributary.",
    digits = 2
    )
...
kable(
    mean,
    caption = "Table S8: Mean, standard deviation, median, 25th, and 75th quartile of evenness of evenness per locus for individuals grouped by run type & tributary.",
    digits = 4
    )
...
kable(
  mean,
  digits = 6,
  caption = "Table S9: Comparison of mean +/- standard deviation, minimum, and maximum values of the nucleotide diversity (pi) across loci among individuals grouped by run within tributary."
)
...
kable(
    mean,
    digits = 2,
    caption = "Table S10: Comparison of mean, median, 25th and 75th quantile of global diversity at fixed loci for individuals grouped by run/tributary."
    )
...
kable(
    null_dist %>%
        ...
        select(-sign),
    digits = 2,
    caption = "Table S11: Chromosomes with significantly higher/lower than expected (outside simulated null distribution) number of fixed loci for each run/tributary group."
    )
...
kable(
    pattern %>%
        ...
        summarize(mean = mean(percent),
                  ...
                  max = max(percent)),
    digits = 2,
    caption = "Table S12: Mean +/- sd, minimum, and maximum percent of loci per chromosome that are fixed across a group of individuals with the same run type for a given tributary."
    )
...
kable(
    mean,
    digits = 2,
    caption = "Table S13: Mean, standard deviation, median, 25th and 27th quantile of the number of singletons observer per individual for individuals grouped by run/tributary."
    )
...
kable(
      pattern %>%
          ...
          summarize(mean = mean(percent),
                    ...
                    max = max(percent)),
      digits = 2,
      caption = "Table S14: Comparison across run/tributary group of the mean +/- sd, minimum, and maximum percent of loci on a chromosome that are only polymorphic for a set of individuals from the same tributary of the same run type."
      )
...
kable(
    sign %>%
      count(GRP, sign) %>%
      arrange(desc(n)),
    caption = "Table S15: Number of chromosomes with significantly more/less loci with private polymorphisms than expected."
    )
...
kable(
    sign %>%
      ...
      arrange(desc(n)),
    caption = "Table S16: Number of run/trib groups a chromosome has been flagged as having significantly more/less loci with private polymorphisms than expected."
    )
...
kable(
  alleles_private %>%
    ...
    ungroup() %>%
    arrange(desc(max)),
  caption = "Table S17: Summary stats of number of individuals carrying private alleles (individuals grouped by run type within tributaries."
)
